...
<div class="space-y-4 grid grid-cols-3">
    ...
    <div class="mb-4 pr-4">
        ...
        <input id="document" type="tel" name="document" value="{{old('document', $user->document ?? '')}}" required
               data-mask="cpf"
               class="w-full border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm  @error('document') border-red-500 @enderror">
        @error('document')
        <span class="text-red-500 text-sm">{{ $message }}</span>
        @enderror
    </div>
...
<div class="space-y-4 grid grid-cols-3">
    ...
    <div class="mb-4 pr-4">
        ...
        <input id="position" type="text" name="position" required value="{{old('position', $user->position ?? '')}}"
    ...
    <div class="mb-4 pr-4">
        ...
        <input id="birth_date" type="text" name="birth_date" required value="{{old('birth_date', isset($user->birth_date) ? \Carbon\Carbon::parse($user->birth_date)->format('d/m/Y') : '')}}"
               data-mask="date"
               class="w-full border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm  @error('birth_date') border-red-500 @enderror">
        @error('birth_date')
        <span class="text-red-500 text-sm">{{ $message }}</span>
        @enderror
    </div>
